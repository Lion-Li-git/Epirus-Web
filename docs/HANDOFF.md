...
待办 ①（镜面反射"空指"语义）**已完成**（v1.5.16，见下）⇒ 下一项 = **② 策略要能"选蓄哪种珠"**。
上线冠军仍是 **`eco-34`**（未换；成绩已按新规则用 `promote-champion` 重测重记）；版本号 **v1.5.16**；最近提交 = v1.5.16 那条（`git log -1`）。

## 1. 待办队列（按顺序做；每做完一项跑：spec + np-test + smoke + battle-test）
### ✅ ① 镜面反射的"空指"语义（**v1.5.16 已完成**；规则条文 `docs/RULES-NP.md` N14）
- **实现**（`js/core/resolve.js`）：`applyMirrorSelf`（非伤害类技能的自效果表）、`mirrorRefs`（t1/t2 单点真源，
  结算/架势/无效化/连带四处共用）、`effectiveKeyOf`（镜面反射的"有效技能"= 它复制到的那张卡）、
  `guardKindOfKey` + `copiedGuardKind`（技能→架势的唯一真源）；大雷快照加 `snapT2`（连带认**两个**被作用者）。
- **关键裁定**：复制来的架势在**声明那一刻**生效 ⇒ 能挡同回合的大雷（这是用户例子的硬要求）；
  复制**蓄能**的珠类型取**复制者自己**的选择（不读 t1，避免 v1.5.15 刚修掉的信息泄漏复活）。
- **守门 `np-test D23`**（7 组）：真架势 / 用户例子端到端 / 复制ジ / 珠类型不泄漏 / t1 被作废=唯一"无可复制" /
  **全部 30 个技能穷举归类**（dmg 复制 / 自效果 / 仅空指）/ 每个自效果技能真复制一遍。反证三次全红
  （① 自效果表失效 ⇒ D23③ 红 ② `guardOf` 只读 `a.key` ⇒ D23① 红 ③ 去掉 `snapT2` ⇒ D23② 红）。
...
- 规则指纹：**`e2241072`**（v1.5.16 改了 `rules.js` / `resolve.js`）。训练锁：`docs/artifacts/.training.lock`
