...
7. ~~`全息屏障` 的实现与设计意图不一致~~ ⇒ **✅ 已裁定并修正（v1.5.4）**：原始规则集（老仓库
   `D:\code\Epirus\README.md`「全息屏障」）写明「给**被作用者**施加一个"原型制御"」、手势指向被作用者、
   且「雷击之枪需要对**使用者**作用才能使其无效」⇒ 它是**对别人用的盾**。已改 `rules.js` 的
   `target:'other'` + `resolve.js` 的 `holoShieldFrom()`（被套盾者获得原型制御式架势 ⇒ 格挡/转移/豁免/禁用
   全部自动复用），旧 ⚠️ R18（仅限自己）作废；守门 `np-test D14`（**三次反证都红**）。
   **v1.5.5 补了第二处（用户指出）**：盾必须经 `actionOf()` 判定（它跳过 `voided`），**不能直读 `state.actions`**
   —— 直读会漏掉"被小雷无效化"。原始规则「优先级原则」（未标注=3、优先级高的阻止优先级低的）⇒ 小雷(5)先结算
   ⇒ **打施放者即可拆盾**（D14 ⑤），打被套盾者拆不掉（D14 ⑥）。
   **该卡旧语义下的所有读数作废**，要按新语义重测。
   用户的另一条裁定（仍有效）：**结盟式打法不必强求** —— 当前策略没有结盟建模，
   所以"AI 不会用这张卡"是**预期行为**，不要按"AI 该学会它"处理。
8. ~~`全息屏障` 在 2P 模式下成了陷阱卡~~ ⇒ **✅ 已裁定并修掉（v1.5.7）**：用户选"**2P 灰掉这张卡**"
   （理由：2P 的自保本来就有原型制御、效果完全相同 ⇒ 零损失；另一个选项"2P 继承新语义留着死卡"被否）。
   实现：`rules.js` 的 `MULTI_ONLY` 加 `SK.HOLO`；守门 `np-test D14` ④b。
9. ~~5P 线上冠军被 v1.5.4 的语义修正打坏~~ ⇒ **✅ 已换成 `long-33`（v1.5.7，用户裁定）**：
   旧的 v1.3.58 产物在同一考卷上 38.0%→15~17.8%（`--ban=holo` 回到 38.6%，缺口全来自这张卡）；
   换上的 long-33 在 canonical 多人 3 血考卷上 **41.0%**（旧 17.8%）、同场 5 血口径 59.9%。
   **两条规矩已落地**：① 规则指纹守门 —— `tools/rules-fingerprint.mjs` + bundle meta 记
   `rulesFingerprint`/`examScoreAtBuild` + **`np-test D16`（指纹不符立刻红）**；
   ② 「某卡有无实战影响」**不能**由技能报告（富经济单场）判，必须用**训练口径 A/B** 判。
10. `蓄能` desc 曾写"可累加"与 R9' 矛盾 —— **已在 v1.4.7 修掉**（现仅剩解释性注释），千问体检 §5-5 那条是过期的。
11. **奖励机制内部有冲突（v1.5.7 实测，待用户定修法）**：恢复的技能熵奖励 `divW=0.06` 与 ep 攒钱奖励**互相打架**
    —— 熵按**动作分布**算，而"攒钱"就是反复出ジ ⇒ 熵极低 ⇒ 熵奖励在惩罚攒钱。
    实测（`rwd0` 旧门槛 vs `eco` 新门槛，同一引擎）：最高 ep **44.8→12.8**、ep≥3 决策 **34.3%→5.8%**、
    出手种类 **13.7→14.8**，标准考卷 +3.57pt（不显著）。
    ⇒ 修法（二选一）：(a) `divW` 降到 0.006 量级；(b) **熵只统计"非ジ"的动作**。
    ⚠️ 现行在库冠军不受影响（都在此改动之前训练）。
12. **训练池加"单一防御 specialist"是有效杠杆**（v1.5.7 实测，**修正 v1.5.2 的旧结论**）：
    池子加 `reflectspam/guardspam/protowall`（12→15 对手）后，长程反弹墙从"2/6 seed 拿 **0%**"
    变成"5/6 拿 100%、最差 60%"，标准考卷 +1.08pt（没掉）。
    ⇒ 精确结论：**加"复杂策略对手"是弱杠杆（v1.5.2/v1.5.3 两次），加"见过就能反制的单一 specialist"不是**。
    判据要看**有没有灾难点**，别看均值（该场双峰、sd≈58）。详见 `docs/REVIEW-5P.md` §10。
