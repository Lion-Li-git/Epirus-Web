...
8. **`全息屏障` 在 2P 模式下成了陷阱卡**（v1.5.6 体检发现，**需要裁定**）：
   v1.5.4 把它的目标从 `self` 改成"任一其他玩家"后，**2P 里唯一的"别人"就是对手** ⇒ 出这张卡 = 给对手套盾。
   冻结版 v1.0.0（标签 `v1.0.0`）当年是自保（⚠️ R18，已废）。三个选项：
   (a) 2P 也继承新语义（承认它是多人/结盟向的卡，2P 里等于死卡）；(b) 2P 单独保留自保（模式特例）；
   (c) 2P 允许指自己**或**对手（保留卡的可用性，同时不违背"被作用者"的字面）。
   ⚠️ 已查：**2P 线上冠军 400 局里 0 次使用它** ⇒ 无论选哪个都不会动到现有 2P 产物。
9. **5P 线上冠军被 v1.5.4 的语义修正打坏**（千问体检报告 §3，本轮已独立复现，**需要裁定**）：
   权重 md5 未变，但同一命令（`eval-5p.mjs 20 5 77000`）从 **38.0% 掉到 15.0%**；
   `--ban=holo` 拿掉这张卡后回到 **38.6%** ⇒ 缺口全来自这一张卡（网络把它当"1 费自保"选，现在它护别人）。
   同场 `long-33` 拿 **59.9%**（报告 250 局口径 45.3%）⇒ 库里有更强的产物。
   选项：(a) 换线上冠军（long-33 或重训一个 3 血冠军）；(b) 保留并明确写"线上是 3 血族、长程挂长程冠军"。
   **教训（要变成规矩）**：①「某卡无实战影响」**不能**由技能报告（富经济单场）判，必须用训练口径 A/B 判；
   ② **改 `rules.js` 的 `target`/`pierce`/`cost` 任一字段 = 改所有已训练产物的输入分布** ⇒
   必须重跑线上冠军考卷，并把"修前/修后"数字一起写进 CHANGELOG。建议再加机械守门：
   bundle meta 里记 `rulesFingerprint` + `examScoreAtBuild`，指纹不符的工具/页面直接报"成绩已过期"。
