...
## 0. 一句话状态
> **交接 / 验收入口**：`docs/AUDIT-REQUEST-v1.5.17.md` —— 现状 + **可逐条判决的断言清单（A1~A10，含反证做法）**
> + 复核命令手册 + 预期目标 + 未决事项 + **我可能错的地方（R1~R6）**。新会话建议先读它。
**规则层正在修 bug，训练暂停**（用户裁定："我认为你可能需要先修好规则再训练"）。
待办 ①（镜面反射"空指"语义 + 生效时机）**已完成**（v1.5.16 实现、v1.5.17 按用户二次裁定修正）⇒ 下一项 = **② 策略要能"选蓄哪种珠"**。
上线冠军仍是 **`eco-34`**（未换；成绩已按新规则用 `promote-champion` 重测重记）；版本号 **v1.5.17**；最近提交 = v1.5.17 那条（`git log -1`）。

## 1. 待办队列（按顺序做；每做完一项跑：spec + np-test + smoke + battle-test）
### ✅ ① 镜面反射的"空指"语义（**v1.5.16 完成 / v1.5.17 修正**；规则条文 `docs/RULES-NP.md` N14）
- **v1.5.16**：没有"无可复制" —— 非伤害类技能的效果复制到**使用者自己**身上（架势 / 蓄能 / 聚能环 / 避雷针 /
  地雷 / 净化），t1 行动被作废时才是唯一的"无可复制"。
- **v1.5.17（用户二次 + 三次裁定，推翻 v1.5.16 的一处实现）**：**大雷 pri4 先于镜面反射 pri3 结算**，大雷会
  **无效化**非防御类技能（镜面反射不算防御族）⇒ **复制根本不会发生**（用户原话："反弹并没有被复制成功，是我错了"）。
  而**同优先级时防御类先出现** ⇒ 复制来的防御架势挡得住同优先级的 激光剑 / 坦克（按原本伤害矩阵）
  与随后的 ⑤ 枪 / ⑥ 狙击，挡不住先前的大雷 / 小雷。
  实现：新增 `mirrorGuardPass`（在 ④ 之前只落防御族的架势）+ 状态字段 `p.copiedGuard`（`startTurn` 清空）；
  `snapT2`（空指参与连带）已撤回。
- **实现**（`js/core/resolve.js`）：`applyMirrorSelf`（自效果表）、`mirrorRefs`（t1/t2 单点真源）、
  `guardKindOfKey` + `copiedGuardKind`（技能→架势的唯一真源）。**守门 `np-test D23`**（8 组，含 30 技能穷举归类）。
- 反证三条（实测全红）：架势改回"声明派生"+"大雷豁免镜面反射" ⇒ D23② 红；自效果表失效 ⇒ D23③ 红；
  t1 被作废仍去复制 ⇒ D23⑤ 红。
- 详见 `CHANGELOG.md` v1.5.16 / v1.5.17。
...
- **参数量评估见 `docs/PARAMS-PLAN.md`**（用户 2026-09-13 要求："整理冠军所有问题、哪些能靠加参数量解决；要加就一次全加上"）。
  要点：兼容机制**可以复用**（`shapeOf` 从 `params.length` 反推形状 + `features()` 裁掉旧版没有的维度；
  新维度**追加在末尾**即可让旧存档在**工具侧**继续可评测）—— 但游戏侧 `unpack(o,false)` 是严格校验，
  **线上那一包仍须重训重发**。真正卡在"表达力"上的只有 4 项（珠类型 / 镜面反射复制到了什么 / 自身架势 / 目标选择），
  前 3 项合计 +72~120 参数（约 +2~4%）；第 4 项（"打谁"完全不学习）建议单独一版。
...
## 2. 本会话已做完并已推送的（**别重做**）
- **v1.5.16** 镜面反射回归原始规则（**没有"无可复制"**）：非伤害类技能效果复制到自己身上 + 对 t2 空指
  （空指参与大雷连带）；复制来的架势在声明那一刻生效；守门 `np-test D23`（见上方 ① 的 ✅ 块）。
...
- **v1.5.16 新增候选（下次特征版本一起做）**：特征**看不到"镜面反射复制到了什么"** —— 对手槽位的
  `lastSkill` 只显示 `mirror`，看不到"有效技能"（复制来的架势本身也没进特征）。
  要补就得动 `js/train/policy.js` 的槽位布局 ⇒ `FEAT_S` +1、**旧冠军包全部不兼容** ⇒
  与 TODO ②（策略的"珠类型输出位"）同批做，别单独插队。

## 4. 关键基线数字
- 上线冠军 `eco-34`：多人 3 血考卷 **41.4%（40 局）**、自对局 13.6 伤害/局、0% 平局、28.5 回合、全息屏障 0。
  （v1.5.16 改规则后已按 §1④ 的铁律用 `promote-champion` 重测重记：成绩不变，`examScoreAtBuild = 0.414`。）
- 回归：`spec 37/37`、`np-test 72/72`（D1–D23）、`smoke SMOKE OK`、`battle-test BATTLE OK`。
- 规则指纹：**`8cd4d731`**（v1.5.17 改了 `resolve.js` / `state.js` / `rules.js`）。训练锁：`docs/artifacts/.training.lock`
  （训练期间别跑 D16/champ-audit/battle-test）。
...
18. **"冠军的行为"与"某个风格脚本的行为"必须分清**（v1.5.13，用户实测踩到）：多人局难度下拉的**默认**是
    `st:combocounter`（读招反制）**脚本**，不是冠军 ⇒ 用户拿它当"冠军"看，于是"枪使用率很高"（实测该风格
    **枪 34.3%**、激进快攻 **49.3%**）与 skill report 对不上；冠军 `eco-34` 的枪使用率是 **0%**
    （真实对局与 skill report 都是 0%，它用激光剑 11.6% / 狙击枪 11.2%）。⇒ 想看冠军就把难度选成
    「**冠军（最强）**」；skill report 的「使用率」列是**富裕经济**口径（每回合把 ep 补到 10），
    与真实对局（原生经济）不同 —— 它的「Δ_lost 强度」列才是主口径，真实对局的使用率看
    `node tools/eval-5p.mjs 40 5 77000 <包>` 末尾的分布块。
19. **规则文档里"对谁"没写清时，实现很容易只对 2 人局正确**（v1.5.13 修用户报的狙击枪 bug）：
    ...
    2 人局恰好等价（只有两个人 ⇒ 目标的枪必然指向狙击手，且枪 pri2 > 狙击 pri1 本来就由优先级规则阻止）
    ⇒ `spec R13/57` 一直绿、多人局才错。守门 `np-test D20`。**教训**：规则条目里凡是出现
    "该玩家/攻击者/被作用者"这类指代，实现处要写一句"到底指向谁"，并配一条**多人局**用例
    （2 人局用例覆盖不到这类歧义）。
20. **"按模式猜默认值"必须先证明该模式真的会触发那条规则**（v1.5.12 回滚 v1.5.11 的"长程默认开哨声惩罚"）：
    干净配对（`lngC` 自动 0.5 vs `lngD` 显式 0，6 seed，其余全同）产出**逐字节相同** ⇒ 长程里它是**空操作**
    （长程局几乎总以淘汰结束 ⇒ `aliveEnd>=2` 的哨兵局≈0%）。验证成本 20 分钟；不验证的话线上会长期挂着
    一个"看起来在防摆烂、其实从未生效"的开关。
