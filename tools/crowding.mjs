...
function run(me, k, mode, seed0) {
  ...
  for (let g = 0; g < N; g++) {
    ...
    const cs = []; for (let i = 0; i < 5; i++) cs.push(mine.indexOf(i) >= 0 ? me : rnd);
...
for (const f of FILES) {
  ...
  console.log(`  ${f.split('/').pop().replace('.bak', '').replace('bundled-champion-3p.js', '线上包').padEnd(18)}${a.perSeat.toFixed(1).padStart(6)} ${b2.perSeat.toFixed(1).padStart(8)} ${b4.perSeat.toFixed(1).padStart(8)}        ${idx.toFixed(2)}          ${b4.draw.toFixed(0)}%`);
}
