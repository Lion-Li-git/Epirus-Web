  ...
  function chooseAIMulti(state, pid, legal) {
    ...
    if (B.diff === 'champ') {
      ...
      if (c) {
        ...
        /* ===== v1.5.139（用户 09-21 晨裁定：ε=0.25 全候选"昏手太多"，要均匀但更小）=====
         * 浏览器冠军 = softmax(0.15) + **ε=0.4 的"键级探索"**（探索时只在网络打分前 5 的**不同技能键**
         * 里均匀选，键内取最优候选——目标仍由网络说了算）。机制与实测表在 evo.js `policyChooserN`。
         * 对照（headless · 93 权重）：2 席长程镜像 ε=0 ⇒ 104 回合 0% 决胜；ε.25 全候选 ⇒ 56 回合/97%
         * 但空爆 1.07/局（用户看到的贴贴/天火昏手）；**ε.4~.5 top5 键 ⇒ ~30 回合/100% 决胜、空爆 0.13/局**。
         * 温度/eval 路径不动：训练与门禁调用 eps=0 ⇒ 读数逐字不变（全仓 eps>0 的只有这一行）。 */
        return finish(Trainer.pickChampion(state, pid, legalForAI, c, 0.15, 0.4));
      }
